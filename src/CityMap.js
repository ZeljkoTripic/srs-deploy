...
function CityMap() {
    ...
    const events = [{ cont: "studentski dom", x: 44.7658380659706, y: 17.200942913300736, icon:true}, { cont: "event2", x: 44.73, y: 17.56,icon:false }, { cont: "event3", x: 44.53, y: 17.21, icon:false }];
    return (
        <div className="mapArea">
            <MapContainer center={[44.78798121640895, 17.201115245677336]} zoom={12} scrollWheelZoom={true}>
                ...
                />
                {events.map(event => (event.icon ? addMarkers(event.cont, event.x, event.y,"./work.png") : addMarkers(event.cont, event.x, event.y,"./danger.png")))};
                {addMarkers("desavanje1", 44, 17,"./work.png")};
...
function addMarkers(event, x, y,icon) {
    ...
    return (
        <Marker position={[x, y]} icon={markerIcon}>;
            <Popup>
                {event} <br /> Easily customizable popup.
            </Popup>
        </Marker>
